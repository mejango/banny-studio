...
@Test(.enabled(if: ep1Exists)) func importsRealEp1Staging() throws {
    ...
    #expect(doc.settings.activeScene == 1)

    // Active scene ("Scene 1"): SAGE, HELIUS, TED. Scene 0 ("Scene 2") carries the SAGE audio track.
    let scene1 = doc.scenes[doc.settings.activeScene]
    #expect(scene1.state.characters.map(\.name) == ["SAGE", "HELIUS", "TED"])
    #expect(doc.scenes[0].state.audioTracks.contains { $0.name == "SAGE" })

    // DARL carries the 923-event performance, sorted by time.
    let darl = try #require(doc.scenes.flatMap(\.state.characters).first { $0.name == "DARL" })
    ...
    // Known clip from the inventory.
    let clip = try #require(doc.scenes.flatMap(\.state.characters).flatMap(\.clips)
