...
func run() throws {
    ...
    case "pack":
        guard args.count >= 4 else { throw CLIError.usage("banny pack <folder.bs> <out.bs>") }
    ...
    case "unpack":
        guard args.count >= 4 else { throw CLIError.usage("banny unpack <in.bs> <out.bs>") }
        let out = URL(fileURLWithPath: args[3])
        guard !FileManager.default.fileExists(atPath: out.path) else {
            printErr("error: \(out.path) already exists"); exit(1)
        }
    ...
    default:
        print("""
        usage: banny <command>              (a .bs project is a folder or a zip; both work)
          catalog [--json]                                — wardrobe options (bodies, outfits, eyes, mouths)
          new <out.bs> [--characters N]                   — create a starter project (folder)
          ...
          unpack <in.bs> <out.bs>                         — extract a zipped .bs for editing
